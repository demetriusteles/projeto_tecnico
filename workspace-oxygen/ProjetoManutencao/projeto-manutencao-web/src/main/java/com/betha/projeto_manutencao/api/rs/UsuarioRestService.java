...
public class UsuarioRestService {
    ...
    public Response login(@HeaderParam("Authorization")
        ...
        return Response.status(status)
        		.entity(new ApiResponse<>(status.getStatusCode(), status.toString(), service.login(authorization)))
    ...
    @PermitAll
    @Produces(MediaType.APPLICATION_JSON)
	public Response create(final UsuarioResponseDTO dto) throws AccountException {
        Response.Status status = Response.Status.OK;
        return Response.status(status)
